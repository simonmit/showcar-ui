        ...
        setHideSample: function (callback) {
            ...
            setTimeout(function () {
                if (_this.$el.querySelector('.ndpl-component__code')) {
                     ...
                     */
                    if (_this.$el.querySelector('.ndpl-component__sample').offsetHeight <= 74 && ! _this.component.options.disabled_auto_sample_hiding) {
    ...
    methods: {
            ...
            _this.$http.get('./docs/data.json' + '?cb=' + new Date()).then(function (response) {
                _this.initData(response.data, function () {
                    if (_this.$data.groups.length) {
                        ...
                        _this.logInfo('You need to add a component to your library before it can be loaded.<br/>You can either do this manually by editing your <code>data.json</code> file,<br/> or you can use the command line helper: <code>astrum new [group_name/component_folder]</code>');
                    }
                });
            });
        },
        ...
        setupComponents: function () {
            ...
            for (var i = 0; i < _this.groups.length; i ++) {
                ...
                for (var j = 0; j < group.components.length; j ++) {
                    ...
                    _this.$set('groups[' + i + '].components[' + j + '].js', '');
        ...
        loadComponent: function (component) {
            ...
            if (component.subgroup) {
                component_folder = component.subgroup;
            } else {
