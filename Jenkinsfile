  ...
  stages {
    ...
    stage('DeployBranch') {
      ...
      steps {
        ...
        sh './deploy/deploy.sh'
        slackSend channel: 'as24_acq_cxp_fizz', color: '#FFFF00', message: ":question: Showcar-UI branch ${env.BRANCH_NAME} is deployed, check it with toggle (<${env.BUILD_URL}|Open>)"
